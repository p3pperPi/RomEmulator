...
return activity_snooped;
 ...
 //----------------------------------------------------------------------------------------------------------------------------------------
 //----------------------------------------------------------------------------------------------------------------------------------------
 // uint8_t (*my_ack_function)(uint8_t)
 // [called in get_byte]
 //  arg : received data (ROW data / NOT SHIFTED)
 //  ret : ACK(!=0)/NACK(==0)
 //
 ...
 // Ack or Nack by device address data from master
 // arg : device address (8Bit)
 // ret : ACK(!=0)/NACK(==0)

 // uint8_t (*fp_respond_to_command)(uint8_t chip_address),
 // Ack or Nack by command from master
 // called only R/W bit == W(0) in address byte
 // arg : command (8Bit)
 // ret : ACK(!=0)/NACK(==0)

 // uint8_t (*fp_respond_to_data)(uint8_t chip_address),
 // Ack or Nack by command from master
 // called only R/W bit == W(0) in previous byte
 // arg : data (8Bit)
 // ret : ACK/NACK
 ...
 // Generate data sending to master
 // Note - DO NOT have bit counter -> need to count number of data in this function
 // arg : address (7Bit) , sending data to master (need to change in this function)
 // ret : number of remaining data

 // uint8_t (*fp_receive_byte)(uint8_t chip_address, uint8_t byte)
 // Treat received value
 // Note - DO NOT have bit counter -> need to count number of data in this function
 // arg : address (7Bit) , received data from master
 // ret : ACK/NACK
